...
APM.prototype = {
  ...
  parse( buffer, offset ) {
    ...
    if( buffer.readUInt16BE( offset + 0 ) === APM.DriverDescriptorMap.signature ) {
      this.driverMap = this.driverMap ?
        this.driverMap.parse( buffer, offset ) :
        APM.DriverDescriptorMap.parse( buffer, offset )
    } else {
      this.driverMap = null
    }
  ...
  write( buffer, offset ) {
    ...
    var size = ( this.partitions.length + 1 ) * this.blockSize

    buffer = buffer || Buffer.alloc( size )
    offset = offset || 0

    if( this.driverMap ) {
      this.driverMap.write( buffer )
    }

    for( var i = 0; i < this.partitions.length; i++ ) {
      this.partitions[i].write( buffer, ( i + 1 ) * this.blockSize )
    }
